...
import Link, { type LinkProps } from '@watanuki/core/link';
...
export function SidebarSeparator(props: ComponentProps<'p'>) {
  ...
  return (
    <p
      {...props}
      className={cn(
        'inline-flex items-center gap-2 mb-1.5 px-2 mt-6 empty:mb-0 text-xs font-medium text-fd-muted-foreground/70',
        depth === 0 && 'first:mt-0',
        props.className,
      )}
    >
...
export function SidebarFolder({
  ...
  return (
    <Collapsible open={open} onOpenChange={setOpen} disabled={!collapsible} {...props}>
      <FolderContext
        value={useMemo(
          () => ({ open, setOpen, depth, collapsible, active }),
          [active, collapsible, depth, open],
        )}
      >
        {children}
      </FolderContext>
