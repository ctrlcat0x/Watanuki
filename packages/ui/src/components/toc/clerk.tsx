...
export function TOCItems({ ref, className, children, ...props }: TOCItemsProps) {
  ...
  const onPrint = useCallback(() => {
    ...
    output.unshift(
      <path key="path" d={d} className="stroke-fd-primary" strokeWidth="2" fill="none" />,
    );
...
export function TOCItem({
  ...
  return (
    <Primitive.TOCItem
      href={item.url}
      {...props}
      data-selected={selectedUrl === item.url}
      className={cn(
        'prose relative py-1.5 text-sm scroll-m-4 text-fd-muted-foreground hover:text-fd-accent-foreground transition-colors wrap-anywhere data-[selected=true]:text-fd-primary',
        isFirst && 'pt-0',
        isLast && 'pb-0',
        props.className,
      )}
