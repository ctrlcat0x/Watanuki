...
const SCREEN_X = 1;
/** Flush under white toolbar (ends at y=51) — avoids dark chrome strip at y=51–52. */
const SCREEN_Y = 51;
const SCREEN_WIDTH = 1200;
/** 51 + 701 = 752 — matches rounded bottom edge. */
const SCREEN_HEIGHT = 701;
...
const MEDIA_FILL = '#d4d4d4';

/** Screen punch — starts at toolbar bottom (51), not 52. */
const SCREEN_PUNCH =
  'M1 51H1201V741C1201 747.075 1196.08 752 1190 752H12C5.92486 752 1 747.075 1 741V51Z';
...
export function Safari({
  ...
  const punchId = `${uid}-safari-punch`;
  const path0Id = `${uid}-safari-path0`;
  const roundedId = `${uid}-safari-rounded`;
  ...
  return (
    <div
      className={cn('not-prose relative inline-block w-full align-middle leading-none', className)}
      style={{
      ...
      {!hasVideo && imageSrc && (
        ...
        >
          <img
            src={imageSrc}
            alt=""
            className="absolute inset-0 !m-0 block size-full border-0 object-fill object-top p-0"
            style={{ margin: 0 }}
            draggable={false}
          />
        </div>
        ...
        <defs>
          <mask id={punchId} maskUnits="userSpaceOnUse">
            <rect
              ...
              fill="white"
            />
            <path d={SCREEN_PUNCH} fill="black" />
          </mask>
          ...
          <clipPath id={roundedId}>
            <path d={SCREEN_PUNCH} fill="white" />
          </clipPath>
        </defs>
...
export type BrowserFrameProps = SafariProps;
export const BrowserFrame = Safari;
