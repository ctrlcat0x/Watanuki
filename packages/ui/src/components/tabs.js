...
export function TabsList({ className, ...props }) {
    return (_jsx(Unstyled.TabsList, { ...props, className: (s) => cn('flex gap-3.5 text-fd-secondary-foreground overflow-x-auto px-4 not-prose', typeof className === 'function' ? className(s) : className) }));
}
export function TabsTrigger({ className, ...props }) {
    return (_jsx(Unstyled.TabsTrigger, { ...props, className: (s) => cn('inline-flex items-center gap-2 whitespace-nowrap text-fd-muted-foreground border-b border-transparent py-2 text-sm font-medium transition-colors [&_svg]:size-4 hover:text-fd-accent-foreground disabled:pointer-events-none disabled:opacity-50 data-[active]:border-fd-primary data-[active]:text-fd-primary', typeof className === 'function' ? className(s) : className) }));
}
