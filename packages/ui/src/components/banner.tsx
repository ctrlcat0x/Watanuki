...
export function Banner({
  ...
  // ponytail: base32 keeps class/script injection-safe; escape id for CSS selectors
  const globalKey = id ? `nd-banner-${encodeBase32(id)}` : null;
  const cssSafeId = id ? escapeCssIdent(id) : null;
  ...
  return (
    ...
    >
      ...
      {globalKey && cssSafeId ? (
        <style>{`.${globalKey} #${cssSafeId} { display: none; }`}</style>
      ) : null}
      {globalKey ? (
        <script
...
function escapeCssIdent(value: string): string {
  // ponytail: strip unsafe chars — enough to block #id selector injection (not full CSS.escape)
