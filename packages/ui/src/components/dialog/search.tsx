...
import { ChevronRight, Hash, SearchIcon, X } from 'lucide-react';
import {
  type ComponentProps,
  createContext,
  Fragment,
  ...
  return (
    <>
      ...
      {search.length > 0 && (
        <button
          type="button"
          aria-label={t('Clear Search', { note: 'aria-label' })}
  ...
  return (
    <Dialog.Portal>
      <Dialog.Popup
        ...
        className={(s) =>
          cn(
            'fixed left-1/2 top-4 md:top-[calc(50%-250px)] z-50 w-[calc(100%-1rem)] max-w-2xl -translate-x-1/2 rounded-xl border bg-fd-popover text-fd-popover-foreground shadow-2xl overflow-hidden data-closed:animate-fd-dialog-out data-open:animate-fd-dialog-in focus-visible:outline-none',
...
export function SearchDialogList({
  ...
  return (
    ...
    >
      <div
        className={cn(
          'w-full flex flex-col overflow-y-auto max-h-[min(28rem,70vh)] p-1',
          !items && 'hidden',
        )}
      >
        <ListContext
