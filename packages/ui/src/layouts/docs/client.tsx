...
export function useDocsLayout() {
  ...
  if (!context)
    throw new Error(
      'Please use <DocsPage /> (`@watanuki/ui/layouts/docs/page`) under <DocsLayout /> (`@watanuki/ui/layouts/docs`).',
    );
  return context;
}
      ...
      >
        <slots.sidebar.provider defaultOpenLevel={defaultOpenLevel} prefetch={prefetch}>
          <slots.container {...containerProps}>
            ...
            {/* classic/minimal headers already expose a mobile sidebar trigger */}
            {sidebarEnabled && style === 'modern' && <SidebarFab />}
            {tabMode === 'top' && tabs.length > 0 && (
              <LayoutTabs
                tabs={tabs}
                className="z-10 bg-fd-background border-b px-6 pt-3 lg:px-8 max-lg:hidden"
              />
            )}
            {children}
          </slots.container>
