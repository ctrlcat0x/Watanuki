...
import type * as PageTree from '@watanuki/core/page-tree';
...
function FooterItem({ item, index }: { item: Item; index: 0 | 1 }) {
  ...
  return (
    <Link
      href={item.url}
      className={cn(
        'flex flex-col gap-2 rounded-lg border p-4 text-sm text-fd-muted-foreground transition-colors hover:bg-fd-accent/50 hover:text-fd-accent-foreground @max-lg:col-span-full',
        index === 1 && 'text-end',
      )}
    >
      <div
        className={cn(
          'inline-flex items-center gap-1.5 font-medium text-fd-muted-foreground',
      ...
      <p className="truncate text-fd-muted-foreground/80">
