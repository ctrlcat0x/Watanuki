    ...
    <>
      <SidebarContent {...rest}>
        ...
        >
          ...
          {!isModern && slots.searchTrigger && <slots.searchTrigger.full hideIfDisabled />}
          {tabs.length > 0 && tabMode === 'auto' && <SidebarTabsDropdown tabs={tabs} />}
          {banner}
        </div>
        {viewport}
        {((!isModern && (slots.languageSelect || slots.themeSwitch)) ||
          iconLinks.length > 0 ||
          footer) && (
          <div className="relative z-10 flex flex-col gap-2 p-4 pt-2">
            {!isModern && slots.languageSelect && (
              <slots.languageSelect.root
                variant="secondary"
                popoverSide="top"
                matchAnchor
                className="w-full text-fd-muted-foreground text-start justify-start bg-fd-secondary/50"
              >
                <Languages className="size-4.5 shrink-0" />
                <slots.languageSelect.text />
                <ChevronDown className="ms-auto size-3.5 shrink-0" />
              </slots.languageSelect.root>
            )}
            {!isModern && slots.themeSwitch && <slots.themeSwitch className="w-full" />}
            {iconLinks.length > 0 && (
      ...
      <SidebarDrawer>
        <div className="flex flex-col gap-3 p-4 pb-2">
          <div className="flex items-center gap-3">
            {slots.navTitle && (
              <slots.navTitle className="inline-flex flex-1 text-[0.9375rem] items-center gap-2.5 font-medium" />
            )}
            <SidebarTrigger
              className={cn(
                buttonVariants({
        ...
        {viewport}
        <div className="flex flex-col gap-2 border-t p-4 pt-2 empty:hidden">
          ...
          {slots.themeSwitch && <slots.themeSwitch className="w-full" />}
          {iconLinks.length > 0 && (
            <div className="flex items-center gap-1 text-fd-muted-foreground">
              {iconLinks.map((item, i) => (
                <LinkItem
      ...
      {({ collapsed, hovered, ref: asideRef, ...rest }) => (
        <>
          <div
            ...
            className="sticky top-(--fd-docs-row-1) z-20 [grid-area:sidebar] pointer-events-none *:pointer-events-auto h-[calc(var(--fd-docs-height)-var(--fd-docs-row-1))] lg:layout:[--fd-sidebar-width:var(--wt-sidebar-width)] max-lg:hidden"
          >
            {collapsed && showHoverPeek && (
            ...
            <aside
              ...
              className={cn(
                'absolute flex flex-col w-full inset-s-0 inset-y-0 bg-fd-card text-sm duration-250',
                isClassic
                  ? 'border-e border-fd-border'
                  : 'border-e',
                stretchSidebar ? 'items-stretch' : 'items-end *:w-(--fd-sidebar-width)',
                collapsed &&
                  showHoverPeek && [
          ...
          {showCollapsedPanel && (
            <div
              data-sidebar-panel=""
              className={cn(
                'flex transition-opacity max-lg:hidden',
            ...
            >
              ...
              <SearchTrigger className="rounded-lg w-full" hideIfDisabled />
            </div>
          )}
        </>
      )}
...
function SidebarSeparator({ className, style, children, ...props }: ComponentProps<'p'>) {
  ...
  return (
    <Base.SidebarSeparator
      className={cn(
        'inline-flex items-center gap-2 mb-1 px-2 mt-6 empty:mb-0 text-xs font-medium text-fd-muted-foreground/70 [&_svg]:size-3.5 [&_svg]:shrink-0',
        depth === 0 && 'first:mt-0',
        className,
      )}
      style={{
