...
import { TOC, TOCPopover, TOCProvider, } from './slots/toc';
...
export function useDocsPage() {
    ...
    if (!context)
        throw new Error('Please use page components under <DocsPage /> (`@watanuki/ui/layouts/notebook/page`).');
    return context;
}
export function DocsPage({ full = false, tableOfContent: { enabled: tocEnabled = !full, single, ...tocProps } = {}, tableOfContentPopover: { enabled: tocPopoverEnabled, ...tocPopoverProps } = {}, breadcrumb: { enabled: breadcrumbEnabled = true, ...breadcrumb } = {}, footer: { enabled: footerEnabled = true, ...footer } = {}, toc = [], slots: defaultSlots = {}, children, ...containerProps }) {
    tocPopoverEnabled ?? (tocPopoverEnabled = Boolean(toc.length > 0 || tocPopoverProps.header || tocPopoverProps.footer));
    const slots = {
    ...
    return (_jsx(PageContext, { value: {
            full,
            slots,
        }, children: _jsxs(slots.toc.provider, { single: single, toc: tocEnabled || tocPopoverEnabled ? toc : [], children: [tocPopoverEnabled &&
                    (tocPopoverProps.component ?? _jsx(slots.toc.popover, { ...tocPopoverProps })), _jsxs(slots.container, { ...containerProps, children: [breadcrumbEnabled && (breadcrumb.component ?? _jsx(slots.breadcrumb, { ...breadcrumb })), children, footerEnabled && (footer.component ?? _jsx(slots.footer, { ...footer }))] }), tocEnabled && (tocProps.component ?? _jsx(slots.toc.main, { ...tocProps }))] }) }));
}
