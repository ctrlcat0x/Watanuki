...
import * as PageTree from '@watanuki/core/page-tree';
...
export function getLayoutTabs(tree, { transform = defaultTransform } = {}) {
    const results = [];
    ...
    next(tree);
    if (tree.fallback)
        next(tree.fallback, true);
    return results;
}
export function isLayoutTabActive(tab, pathname) {
    if (tab.$folder) {
        return (PageTree.findPath(tab.$folder.children, (node) => node.type === 'page' && isActive(node.url, pathname)) !== null);
    }
    if (tab.urls) {
...
export function useLinkItems({ githubUrl, links }) {
    return useMemo(() => {
...
export function isLinkItemActive(link, pathname) {
...
export { baseSlots, LinkItem } from './client';
