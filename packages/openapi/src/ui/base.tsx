...
import { remarkGfm } from '@watanuki/core/mdx-plugins/remark-gfm';
...
import type { ShikiFactory } from '@watanuki/core/highlight/shiki';
import type { GeneratedPageProps } from '@/utils/pages/builder';
import type { ParsedSchema } from '@/utils/schema';
import { Markdown } from './components/markdown';
import { Schema, type SchemaUIOptions } from '@watanuki/api-docs/components/schema';
...
export function createOpenAPIPageBase({
  ...
  return function OpenAPIPage(props) {
    ...
    if ('preloaded' in props) {
      ...
      if (!doc)
        throw new Error(
          `[Watanuki OpenAPI] the document ${props.document} is not preloaded, make sure to pass the "preloaded" prop to <OpenAPIPage />`,
        );
  ...
  let content = renderPageLayout(
    {
      operations: operations?.map((item) => {
        ...
        if (!pathItem)
          throw new Error(`[Watanuki OpenAPI] Path not found in OpenAPI schema: ${item.path}`);

        const operation = pathItem[item.method];
        if (!operation)
          throw new Error(
            `[Watanuki OpenAPI] Method ${item.method} not found in operation: ${item.path}`,
          );
      ...
      webhooks: webhooks?.map((item) => {
        ...
        if (!webhook)
          throw new Error(`[Watanuki OpenAPI] Webhook not found in OpenAPI schema: ${item.name}`);

        const hook = webhook[item.method];
        if (!hook)
          throw new Error(
            `[Watanuki OpenAPI] Method ${item.method} not found in webhook: ${item.name}`,
          );
