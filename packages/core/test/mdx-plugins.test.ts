...
let bannerBytes: Uint8Array;

beforeEach(async () => {
  bannerBytes = new Uint8Array(
    await fs.readFile(path.resolve(cwd, './fixtures/banner.png')),
  );
  vi.stubGlobal(
    'fetch',
      ...
      const href =
        typeof input === 'string'
          ...
          : input instanceof URL
            ? input.href
            : input.url;
      if (href.includes('watanuki.dev/banner.png')) {
        return new Response(bannerBytes, { status: 200 });
      }
