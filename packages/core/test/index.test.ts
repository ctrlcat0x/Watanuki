...
test('I18n: Format URL', () => {
  expect(DefaultFormatter.get(new NextURL('https://watanuki.dev/en'))).toBe('en');
  expect(DefaultFormatter.get(new NextURL('https://watanuki.dev/en/test'))).toBe('en');
  expect(DefaultFormatter.get(new NextURL('https://watanuki.dev'))).toBeUndefined();
  expect(
    DefaultFormatter.get(
      new NextURL('https://watanuki.dev/docs', {
        nextConfig: {
          basePath: '/docs',
        },
      }),
  ...
  expect(
    DefaultFormatter.get(
      new NextURL('https://watanuki.dev/docs/en/test', {
        nextConfig: {
          basePath: '/docs',
        },
      }),
  ...
  expect(DefaultFormatter.add(new NextURL('https://watanuki.dev'), 'cn').href).toBe(
    'https://watanuki.dev/cn/',
  );
  expect(
  ...
  ).toBe('https://watanuki.dev/docs/cn/');

  expect(DefaultFormatter.remove(new NextURL('https://watanuki.dev/en')).href).toBe(
    'https://watanuki.dev/',
  );
  expect(DefaultFormatter.remove(new NextURL('https://watanuki.dev/en/test/hello')).href).toBe(
    'https://watanuki.dev/test/hello',
  );
  expect(
    DefaultFormatter.remove(
      new NextURL('https://watanuki.dev/docs/en', {
        nextConfig: {
          basePath: '/docs',
        },
      }),
    ).href,
  ).toBe('https://watanuki.dev/docs/');
});
