...
- 5dec9d0: `useWatanukiLoader()` support other names of the serialized page tree
...
- 7292424: Support MDX preset in Watanuki Core
...
- 15bd183: **[Loader API] Default the type of `plugins` to `LoaderPluginOption[]`**
  ...
  export const source = loader(docs.toWatanukiSource(), {
    baseUrl: "/docs",
    plugins: ({ typedPlugin }) => [
      lucideIconsPlugin(),
      typedPlugin({
...
- 851897c: **Remove `@watanuki/core/sidebar` API**

  why: no longer used by Watanuki UI, and the abstraction isn't good enough.

  migrate: The original component is mostly a wrapper of `react-remove-scroll`, you can use Shadcn UI for pre-built sidebars.

- 4049ccc: **Remove `@watanuki/core/server` export**
  - **`getGithubLastEdit`:** Moved to `@watanuki/core/content/github`.
  - **`getTableOfContents`:** Moved to `@watanuki/core/content/toc`.
  - **`PageTree` and page tree utilities:** Moved to `@watanuki/core/page-tree`.
  - **`TOCItemType`, `TableOfContents`:** Moved to `@watanuki/core/toc`.
...
- 429c41a: **Switch to Shiki JavaScript Regex engine by default**
  ...
  - `@watanuki/core/highlight`: use JS engine by default, drop custom engine support, use Shiki directly instead.

- 5210f18: **Set minimal React.js version to 19.2.0**

  19.2 has multiple crucial updates that can improve Watanuki' performance, and it should work seamlessly on mainstream React.js frameworks.

  As a consequence, Next.js 16 is now the minimal version when using Watanuki UI because Next.js always uses the internal canary version of React.js.

- 42f09c3: **Remove deprecated APIs**
  - `@watanuki/ui/page`:
    - removed `<DocsCategory />`.
    - removed `breadcrumbs.full` option from `<DocsPage />`.
  - `@watanuki/core/search/algolia`: renamed option `document` to `indexName`.
  - `@watanuki/core/search`:
  ...
  - `@watanuki/core/highlight`: remove deprecated `withPrerenderScript` and `loading` options from `useShiki()`.
  - `@watanuki/core/i18n`: removed `createI18nMiddleware`, import from `@watanuki/core/i18n/middleware` instead.
  - `@watanuki/core/source`:
...
- 55afd8a: _Migrate to New Orama Cloud_
  ...
  When using Watanuki' Orama Cloud integration, you need to use the new client instead:

  ```ts
  import { sync } from "@watanuki/core/search/orama-cloud";
...
- cbc93e9: Disable `single` by default on `@watanuki/core/toc` API
...
- ad9a004: **Deprecate `@watanuki/core/server` export**

  It will be removed on Watanuki 16, as some APIs under the `/server` export are actually available (and even used) under browser environment.
...
- d1ae3e8: **Move `SortedResult` and other search-related types to `@watanuki/core/search`**
...
- 67df155: `createFromSource` support async `buildIndex` and Watanuki MDX Async Mode
...
- 817c237: Support search result highlighting.
  ...
  Result nodes now have a `contentWithHighlights` property, you can render it with custom renderer, or a default one provided on Watanuki UI.

## 15.6.9

### Patch Changes

- 0ab2cdd: remove waku & tanstack peer dependency temporarily (see https://github.com/fuma-nama/watanuki/issues/2144)
...
- f8d1709: **Redesigned Codeblock Tabs**
  ...
  The old usage is not deprecated, you can still use them while Watanuki' remark plugins will generate codeblock tabs using the new way.
...
- b4916d2: Move `hide-if-empty` component to Watanuki Core
...
- 961b67e: **Bump algolia search to v5**
  ...
  import { sync } from "@watanuki/core/search/algolia";
  ...
  import type { SharedProps } from "@watanuki/ui/components/dialog/search";
  import SearchDialog from "@watanuki/ui/components/dialog/search-algolia";
...
- acff667: **Deprecate `createFromSource(source, pageToIndex, options)`**
  ...
  import { createFromSource } from "@watanuki/core/search/server";
...
- e45bc67: **Remove deprecated `@watanuki/core/middleware` export**

  **migrate:** Use `@watanuki/core/i18n`.
...
- 9a0b09f: **Refactor import path of `@watanuki/core/search-algolia/server` to `@watanuki/core/search/algolia`**
- d9e908e: Improved usage for `createI18nSearchAPI` (replaced `createI18nSearchAPIExperimental`)
- d9e908e: Replace `@watanuki/core/search/shared` with `@watanuki/core/server`
...
- fd46eb6: Move I18n middleware to `@watanuki/core/i18n`
...
- 09c3103: **Change usage of TOC component**
  ...
  **migrate:**
  ...
  ```tsx
  import * as Base from "@watanuki/core/toc";
...
- 0a377a9: **Support writing code blocks as a `<Tab />` element.**
  ...
  ````mdx
  import { Tabs } from "@watanuki/ui/components/tabs";
...
- f75287d: **Introduce `watanuki-docgen` package.**

  Offer a better authoring experience for advanced use cases.
  - Move `remark-dynamic-content` and `remark-install` plugins to the new package `watanuki-docgen`.
  ...
  **Usage**
  ...
  ```ts
  import { remarkDocGen, fileGenerator } from "watanuki-docgen";
  ...
  **Migrate**
  ...
  ```ts
  import { remarkInstall } from "watanuki-docgen";
  ```
...
- 10e099a: Remove deprecated options from `@watanuki/core/toc`
...
- 321d1e1f: **Move Typescript integrations to `watanuki-typescript`**

  why: It is now a stable feature

  migrate: Use `watanuki-typescript` instead.

  ```diff
  - import { AutoTypeTable } from "@watanuki/ui/components/auto-type-table"
...
- cdff313: **Separate Contentlayer integration into another package**
  ...
  why: As Watanuki MDX is the preferred default source, Contentlayer should be optional.

  migrate:

  Install `watanuki-contentlayer`.

  ```diff
  - import { createContentlayerSource } from "@watanuki/core/contentlayer"
  + import { createContentlayerSource } from "watanuki-contentlayer"

  - import { createConfig } from "@watanuki/core/contentlayer/configuration"
  + import { createConfig } from "watanuki-contentlayer/configuration"
  ```
