...
export interface StaticOptions {
  ...
  /**
   * Pick which i18n index to query (locale key from the exported dump).
   */
  locale?: string;
...
async function loadDB(
  from: string,
  // Shell DB only — `load()` restores schema + Orama language from the dump.
  // Do NOT pass i18n locale codes as `language` (Orama wants "english", not "en").
  initOrama: StaticOptions['initOrama'] = () => create({ schema: { _: 'string' } }),
): Promise<Database> {
...
function getDBCached({ from = `${BASE_PATH}api/search`, initOrama }: StaticOptions) {
  ...
  const result = loadDB(from, initOrama).catch((err) => {
...
export function oramaStaticClient(options: StaticOptions = {}): SearchClient {
  ...
  return {
    ...
    async search(query) {
      ...
      if (!db) return [];
      if (db.type === 'simple')
        return searchSimple(db.db as Orama<typeof simpleSchema>, query, search as never);
