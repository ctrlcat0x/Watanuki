...
declare module 'shiki' {
  interface ShikiTransformerContextMeta {
    /** [Watanuki: rehype-code] run async tasks after process */
  ...
  async function onNode(
    ...
    if (!isLanguageLoaded(lang)) {
      if (lazy && lang in highlighter.getBundledLanguages()) {
        ...
        // Graceful degrade when language missing from Shiki bundle
        lang = 'plaintext';
        if (!isLanguageLoaded('plaintext')) await highlighter.loadLanguage('plaintext' as never);
      }
    }
