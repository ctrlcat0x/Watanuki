...
> & {
  ...
  /**
   * Opt-in KaTeX math (`$…$` / `$$…$$`). Pass `{}` or options to enable `remark-math`.
   * Also set `rehypeKatexOptions` (or `{}`) and import `katex/dist/katex.min.css`.
   */
  remarkMathOptions?: Record<string, unknown> | false;
  /** Opt-in companion for `remarkMathOptions`. Pass `{}` or rehype-katex options. */
