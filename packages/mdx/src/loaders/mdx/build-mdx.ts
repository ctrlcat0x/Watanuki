...
export interface WatanukiDataMap {
  /**
   * [Watanuki MDX] raw frontmatter, you can modify it
   */
  frontmatter?: Record<string, unknown>;

  /**
   * [Watanuki MDX] additional ESM exports to write
   */
  'mdx-export'?: { name: string; value: unknown }[];

  /**
   * [Watanuki MDX] The compiler object from loader
   */
  _compiler?: CompilerOptions;

  /**
   * [Watanuki MDX] get internal processor, do not use this on user land.
   */
...
declare module 'vfile' {
  ...
  interface DataMap extends WatanukiDataMap {}
}
...
export async function buildMDX(
  ...
  if (compiler === 'satteri') {
    throw new Error('Sätteri compiler is not supported in Watanuki. Use the default MDX compiler.');
  }
