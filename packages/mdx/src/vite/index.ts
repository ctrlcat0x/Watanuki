...
export interface PluginOptions {
  ...
  /**
   * Update Vite config to fix module resolution of Watanuki
   *
   * @defaultValue true
   */
...
export default function mdx(
  ...
  const metaPlugin: Plugin = {
    name: '@watanuki/mdx:meta',
  };
  const mdxPlugin: Plugin = {
    name: '@watanuki/mdx:mdx',
  };
  ...
  return [
    {
      ...
      async config(config) {
        ...
        if ('_watanuki_skipViteConfig' in config && config._watanuki_skipViteConfig) return;
        if (!options.updateViteConfig) return;

        return {
          optimizeDeps: {
            include: ['@watanuki/core', '@watanuki/mdx', '@watanuki/ui'],
            exclude: ['shiki/wasm'],
          },
          ssr: {
            noExternal: ['@watanuki/core', '@watanuki/mdx', '@watanuki/ui'],
          },
          resolve: {
            alias: {
