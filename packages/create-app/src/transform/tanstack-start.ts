...
/**
 * Remove paths from the `pages` array in tanstack start vite config.
 */
export function removeTanstackPrerender(sourceFile: SourceFile, paths: string[]) {
  const optionsArg = getTanstackStartCall(sourceFile)
    ?.getArguments()[0]
    ?.asKind(SyntaxKind.ObjectLiteralExpression);
  if (!optionsArg) {
