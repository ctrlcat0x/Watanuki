...
import { addTanstackPrerender, removeTanstackPrerender } from '@/transform/tanstack-start';
...
test('transform tanstack start vite config: remove pages', async () => {
  const sourceFile = await createSourceFile('fixtures/tanstack-vite-config(add-pages).output.txt');
  removeTanstackPrerender(sourceFile, ['/static.json']);
  await expect(sourceFile.getFullText()).toMatchFileSnapshot(
    'fixtures/tanstack-vite-config(remove-pages).output.txt',
  );
});
